Feature: Search for users' data on GitHub to track changes in real time
  ...
  Background:
   ...
   Scenario: Total number of Repos, Followers, Following, and Gists should reflect GitHub data
  ...
  Scenario: Verify number of followers against API
    And API: send GET request to users/GradPolina/followers?per_page=100
    And API: verify status code is 200
    Then GitHub Integration API: verify total number of Following
